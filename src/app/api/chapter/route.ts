...
import { buildChapterPrompt, buildChoicesPrompt, parseChapterResponse, looksTruncated, fallbackChoices, analyzeDialogue, passesDialogueGate } from "@/lib/dna/prompt";
  ...
  try {
    ...
    for (let attempt = 0; attempt < 3 && Date.now() < deadline; attempt++) {
      try {
        ...
        const { wordSharePct, substantiveTurns } = analyzeDialogue(parsed.prose);
        // Keep the most dialogue-forward full-length attempt as the fallback.
        if (wordSharePct > bestShare) {
          bestShare = wordSharePct;
          prose = parsed.prose;
          cardQuote = parsed.cardQuote;
          cardQuoteSpeaker = parsed.cardQuoteSpeaker;
          choices = parsed.choices;
        }
        if (passesDialogueGate(parsed.prose, chapterNo)) break; // beat-aware dialogue gate
        console.log(`Prose dialogue ${wordSharePct}% / ${substantiveTurns} turns below beat ${chapterNo} bar, regenerating`);
      } catch (err) {
    ...
    if (prefetch && !passesDialogueGate(prose, chapterNo)) {
